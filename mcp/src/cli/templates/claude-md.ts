...
export function renderHeraldSection(context: HeraldContext, learnedPatterns?: LearnedPatterns): string {
  ...
  if (learnedPatterns && (learnedPatterns.antipatterns.length > 0 || learnedPatterns.patterns.length > 0)) {
    ...
    learnedSection += `*Inheritance: ${context.user}→${context.project}→${context.org}*\n\n`;
...
export async function fetchLearnedPatterns(
  ...
  try {
    // Cascade: user → project → org (more specific wins)
    const queries = [
      `${cedaUrl}/api/herald/reflections?org=${org}&project=${project}&user=${user}&limit=10`,
      `${cedaUrl}/api/herald/reflections?org=${org}&project=${project}&limit=10`,
      `${cedaUrl}/api/herald/reflections?org=${org}&limit=10`,
    ];
