...
describe('CEDA-49: Session Management Tools', () => {
  ...
  describe('Tool definitions', () => {
    it('should have herald_session_list tool defined', () => {
      ...
      expect(indexContent).toContain('List sessions for a org with optional filters');
    });
  ...
  describe('Tool input schemas', () => {
    it('herald_session_list should have optional org, project, user, status, limit properties', () => {
      // Check that the tool has the expected properties in its schema
      expect(indexContent).toMatch(/herald_session_list[\s\S]*?org.*Filter by org/);
