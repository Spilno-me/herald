...
async function doRefreshAccessToken(): Promise<boolean> {
  if (!CEDA_REFRESH_TOKEN) return false;
...
interface TagSet {
  tags: string[];
  trust: TrustLevel;
  source: 'git' | 'stored' | 'path';  // CEDA-105: removed 'env' - pure git derivation
  propagates: boolean;
  gitInfo?: GitInfo;
}

function deriveTagSet(): TagSet {
  // CEDA-105: Pure git derivation - no env var overrides
  // Git is the only trusted source for org/project

  // 1. Check Git remote (HIGH trust) - primary and preferred
  ...
  // 2. Fallback to path (LOW trust) - only if not in git repo
  return {
    tags: deriveTags(),
    trust: 'LOW',
    source: 'path',
...
interface HeraldContext {
  ...
  user: string;
  trust?: TrustLevel;
  source?: 'git' | 'stored' | 'path';  // CEDA-105: removed 'env'
  propagates?: boolean;
  derived?: boolean;
  derivedFrom?: string;
  storedAt?: string;
...
function loadOrDeriveContext(): LoadedContext {
  // CEDA-105: Pure git derivation - no env var overrides
  // Priority: git (HIGH trust) → stored → path (LOW trust)

  // 1. Always try git derivation first (ADR-001: git is trusted source)
  const tagSet = deriveTagSet();

  // If we got valid tags from git, use them (HIGH trust)
  if (tagSet.tags.length > 0 && tagSet.source === 'git') {
    const user = deriveUser();  // Git user takes priority
    return {
      tags: tagSet.tags,
      user,
      trust: 'HIGH',
  ...
  // 2. Check .mcp.json for stored context (preserve previously derived context)
  const mcpJson = readMcpJson();
  if (mcpJson?.herald?.context?.tags?.length) {
    const stored = mcpJson.herald.context;
    const user = stored.user || deriveUser();
    return {
      tags: stored.tags,
      user,
  ...
  // 3. Fall back to path-based derivation (LOW trust)
...
// Tags from context (git > stored > env > path) - can be refreshed
...
let CONTEXT_SOURCE: 'git' | 'stored' | 'path' | 'verified' = LOADED_CONTEXT.source;  // CEDA-105: removed 'env'
...
// Vault context (for multi-project coordination)
...
async function callClaude(systemPrompt: string, messages: Message[]): Promise<string> {
  ...
  try {
    ...
    return `Claude error: ${error instanceof Error ? error.message : error}`;
  }
...
// CEDA-104: Request timeout to prevent CPU spin on unreachable server
const CEDA_REQUEST_TIMEOUT_MS = 10000; // 10 seconds
...
async function callCedaAPI(endpoint: string, method = "GET", body?: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  // CEDA-104: AbortController with timeout prevents hanging on unreachable server
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), CEDA_REQUEST_TIMEOUT_MS);
  ...
  try {
    ...
    clearTimeout(timeoutId);
    if (error instanceof Error && error.name === 'AbortError') {
      return { success: false, error: `Request timeout after ${CEDA_REQUEST_TIMEOUT_MS}ms` };
    }
...
Examples:
  herald-mcp login                          # Authenticate
  herald-mcp config                         # Get MCP config
  herald-mcp init                           # Setup in project
  herald-mcp upgrade                        # Manage subscription
...
Environment:
  ...
  CEDA_TOKEN          Auth token (from getceda.com dashboard)

Context derived from git:
  org                 git remote (github.com/[org]/repo)
  project             git remote (github.com/org/[project])
...
5. Only proceed if user approves`,
  ...
  },
  {
    ...
    description: "Read status from Herald contexts across domains (project vaults)",
    inputSchema: {
      type: "object",
      properties: {
...
`;
      ...
      case "herald_context_status": {
        ...
        for (const v of vaults) {
          const statusPath = join(HERALD_VAULTS_PATH, v, "_status.md");
      ...
      case "herald_sync": {
        ...
        const errors: string[] = [];  // CEDA-FIX: Track error details
