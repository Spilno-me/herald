...
**Recommended: Global install** (prevents npx process spawning issues)
```bash
...
Org and project default to your folder name. Zero config.
...
**Examples:**
```bash
# Basic setup (zero config - context from git)
herald-mcp init

# Sync latest patterns to CLAUDE.md
herald-mcp init --sync

# Add auto-reminder hooks
herald-mcp init --hookify
```
...
project (team patterns)
  ↓ inherits from
org (org-wide patterns)
```

More specific patterns take precedence. If you have a pattern and your org has the same one, yours wins.
...
| `herald://context` | Current configuration (org/project/user) |
...
| `CEDA_TOKEN` | - | Auth token from getceda.com |

**Context is derived from git** (no env vars needed):
...
Context (org/project/user) is always derived from git. No manual override needed.
...
*Herald v1.36.0 — Pattern memory for AI agents*
