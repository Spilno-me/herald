...
1. `env.HERALD_ORG/PROJECT/USER` in .mcp.json → explicit override
...
async function loadOrDeriveContext(): Promise<{
  ...
  if (process.env.HERALD_ORG) {
    return {
      tags: [process.env.HERALD_ORG, process.env.HERALD_PROJECT].filter(Boolean),
...
- Existing users with `HERALD_ORG/PROJECT/USER` → works unchanged
